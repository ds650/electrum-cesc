...
from electrum_cesc.util import PrintError, ThreadJob
...
class Bleutrade(ExchangeBase):
    def get_rates(self,ccy):
        json = self.get_json('bleutrade.com',"/api/v2/public/getmarketsummary?market=CESC_BTC")
	return {'BTC': Decimal(json['result'][0]['Average'])}
#self.get_json('bleutrade.com',"/api/v2/public/getmarketsummary?market=CESC_" + cur)["result"][0]["Average"]
...
class FxPlugin(BasePlugin, ThreadJob):
    ...
    def get_fiat_status_text(self, btc_balance):
        rate = self.exchange_rate()
        return _("  (No FX rate available)") if rate is None else "1 CESC~%s %s" % (self.value_str(COIN, rate), self.ccy)
