...
from electrum_cesc.wallet import BIP44_Wallet
    ...
    def callback_PassphraseRequest(self, req):
        if self.creating_wallet:
            msg = _("Enter a passphrase to generate this wallet.  Each time "
                    ...
                    "access the cryptoescudos in the wallet.") % self.device
        else:
...
class TrezorClientBase(GuiMixin, PrintError):
    ...
    def address_from_derivation(self, derivation):
        return self.get_address('Cryptoescudo', self.expand_path(derivation))
