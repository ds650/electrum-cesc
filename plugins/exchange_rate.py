...
from electrum_cesc.plugins import BasePlugin
...
#        print json_resp
    ...
    def update_b2c(self):
        ...
        for cur in quote_currencies:
            try:
                quote_currencies[cur] = self.get_json('www.bit2c.co.il', "/Exchanges/CESC" + cur + "/Ticker.json")["ll"]
            except Exception:
                pass
        with self.lock:
    ...
    def update_bv(self):
        ...
        quote_currencies = {}
        try:
            for r in jsonresp["CESC"]:
                quote_currencies[r] = Decimal(jsonresp["CESC"][r])
            with self.lock:
                self.quote_currencies = quote_currencies
        except KeyError:
            pass
    ...
    def update_gc(self):
        ...
        quote_currencies = {}
        try:
            for r in jsonresp["prices"]["CESC"]:
                quote_currencies[r] = Decimal(jsonresp["prices"]["CESC"][r])
            with self.lock:
                self.quote_currencies = quote_currencies
        except KeyError:
            pass
    ...
    def update_kk(self):
        try:
            ...
            pairs = ','.join([k for k in resp_currencies if k.startswith("XCESCZ")])
    ...
    def update_vs(self):
        ...
        for cur in quote_currencies:
            try:
                quote_currencies[cur] = self.get_json('api.vaultofsatoshi.com', "/public/ticker?order_currency=CESC&payment_currency=" + cur)["data"]["closing_price"]["value"]
            except Exception:
                pass
        with self.lock:
            self.quote_currencies = quote_currencies
    ...
    def update_bleu(self):
        quote_currencies = {"BTC": 0.0, "LTC": 0.0, "DOGE": 0.0}
        for cur in quote_currencies:
            try:
...
class Plugin(BasePlugin):
        ...
        if self.config.get('history_rates', 'unchecked') == "checked":
            ...
            if cur_exchange == "CoinDesk":
                ...
                if cur_currency in ("ARS", "EUR", "USD", "VEF"):
                    try:
                        resp_hist = self.exchanger.get_json('api.bitcoinvenezuela.com', "/historical/index.php?coin=CESC")[cur_currency + '_CESC']
                    except Exception:
                        return
                else:
                    return
                ...
                if cur_currency in ("EUR", "USD"):
                    try:
                        resp_hist = self.exchanger.get_json('api.kraken.com', "https://api.kraken.com/0/public/OHLC?pair=CESC"+cur_currency+"&interval=1440")['result']['XCESCZ'+cur_currency]
    ...
    def fiat_dialog(self):
        ...
        if str(self.gui.main_window.base_unit()) == "mCESC":
