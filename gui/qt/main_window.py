...
from electrum_cesc.util import print_error, print_msg
...
from electrum_cesc.wallet import format_satoshis
from electrum_cesc import Transaction
from electrum_cesc import mnemonic
from electrum_cesc import util, bitcoin, commands, Interface, Wallet
from electrum_cesc import SimpleConfig, Wallet, WalletStorage


from electrum_cesc import bmp, pyqrnative
...
class ElectrumWindow(QMainWindow):
    ...
    def load_wallet(self, wallet):
        ...
        title = 'Electrum-CESC ' + self.wallet.electrum_version + '  -  ' + self.wallet.storage.path
    ...
    def show_about(self):
        QMessageBox.about(self, "Electrum-CESC",
            _("Version")+" %s" % (self.wallet.electrum_version) + "\n\n" + _("Electrum's focus is speed, with low resource usage and simplifying Cryptoescudoc. You do not need to perform regular backups, because your wallet can be recovered from a secret phrase that you can memorize or write on paper. Startup times are instant because it operates in conjunction with high-performance servers that handle the most complicated parts of the Cryptoescudo system."))
    ...
    def base_unit(self):
        ...
        return "CESC" if self.decimal_point == 8 else "mCESC"
...
#        if not item: return
    ...
    def create_send_tab(self):
        ...
        grid.addWidget(HelpButton(_('Recipient of the funds.') + '\n\n' + _('You may enter a Cryptoescudo address, a label from your list of contacts (a list of completions will be proposed), or an alias (email-like address that forwards to a Cryptoescudo address)')), 1, 4)
        ...
        grid.addWidget(HelpButton(
                _('Cryptoescudo transactions are in general not free. A transaction fee is paid by the sender of the funds.') + '\n\n'\
    ...
    def do_send(self):
        ...
        if self.gui_object.payment_request:
            ...
            if not is_valid(to_address):
                QMessageBox.warning(self, _('Error'), _('Invalid Cryptoescudo Address') + ':\n' + to_address, _('OK'))
                return
    ...
    def create_receive_menu(self, position):
        ...
        if not multi_select:
            ...
            menu.addAction(_("QR code"), lambda: self.show_qrcode("cryptoescudo:" + addr, _("Address")) )
    ...
    def new_account_dialog(self, password):
        ...
        e = QLineEdit()
        vbox.addWidget(e)
        msg = _("Note: Newly created accounts are 'pending' until they receive cryptoescudos.") + " " \
...
#        block_explorers = ['explorer.litecoin.net', 'block-explorer.com', 'Blockr.io']
        block_explorers = ['explorador.cryptoescudo.net']

        block_ex_label = QLabel(_('Online Block Explorer') + ':')
        grid.addWidget(block_ex_label, 5, 0)
        block_ex_combo = QComboBox()
        block_ex_combo.addItems(block_explorers)
        block_ex_combo.setCurrentIndex(block_explorers.index(self.config.get('block_explorer', 'explorador.cryptoescudo.net')))
        ...
        if self.base_unit() != unit_result:
            self.decimal_point = 8 if unit_result == 'CESC' else 5
    ...
    def plugins_dialog(self):
        from electrum_cesc.plugins import plugins
