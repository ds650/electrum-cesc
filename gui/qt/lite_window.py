...
from electrum_cesc.util import get_resource_path as rsrc
from electrum_cesc.bitcoin import is_valid
...
import re
import time
from electrum_cesc.wallet import Wallet, WalletStorage
import webbrowser
...
from electrum_cesc.version import ELECTRUM_VERSION as electrum_version
from electrum_cesc.util import format_satoshis, age
...
class MiniWindow(QDialog):
    ...
    def __init__(self, actuator, expand_callback, config):
        ...
        self.address_input.setPlaceholderText(_("Enter a Cryptoescudo address or contact"))
        ...
        self.setWindowTitle("Electrum-CESC")
...
class ReceivePopup(QDialog):
    ...
    def setup(self, address):
        label = QLabel(_("Copied your Cryptoescudo address to the clipboard!"))
        ...
        self.setWindowTitle("Electrum - " + _("Receive Cryptoescudo payment"))
...
class MiniActuator:
    ...
    def waiting_dialog(self, f):
        ...
        w = QDialog()
        w.resize(200, 70)
        w.setWindowTitle('Electrum-CESC')
    ...
    def send(self, address, amount, parent_window):
        ...
        if dest_address is None or not is_valid(dest_address):
            QMessageBox.warning(parent_window, _('Error'), 
                _('Invalid Cryptoescudo Address') + ':\n' + address, _('OK'))
            return False
