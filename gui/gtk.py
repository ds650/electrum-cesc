...
from electrum_cesc import mnemonic, pyqrnative, WalletStorage, Wallet

Gdk.threads_init()
APP_NAME = "Electrum-CESC"
import platform
MONOSPACE_FONT = 'Lucida Console' if platform.system() == 'Windows' else 'monospace'

from electrum_cesc.util import format_satoshis, parse_url
from electrum_cesc.network import DEFAULT_SERVERS
...
class ElectrumWindow:
    ...
    def __init__(self, wallet, config, network):
        ...
        title = 'Electrum-CESC ' + self.wallet.electrum_version + '  -  ' + self.config.path
    ...
    def do_send(self, w, data):
        ...
        if not is_valid(to_address):
            self.show_message( "invalid cryptoescudo address:\n"+to_address)
            return
