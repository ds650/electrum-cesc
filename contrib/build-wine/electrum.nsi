...
;Variables
  ...
  !define PRODUCT_WEB_SITE "https://github.com/pooler/electrum-cesc"
...
Section
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-cesc.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\electrum-cesc.exe" "" "$INSTDIR\electrum-cesc.exe" 0
  ...
  WriteRegStr HKCU "Software\Classes\cryptoescudo\shell\open\command" "" "$\"$INSTDIR\electrum-cesc.exe$\" $\"%1$\""
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\Classes\cryptoescudo"
